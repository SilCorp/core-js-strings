...
function removeLastOccurrences(str, value) {
  ...
  charsArr.splice(lastIndex, value.length);
...
function countVowels(str) {
  ...
  return str.split('').filter((char) => vocabulary.includes(char)).length;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((char) => cesarMap.get(char) || char)
    .join('');
}
